...
def preprocess_image(image):
    ...
    # 3. Upscale if the image is too small for better character recognition
    if image.width < 2200:
        scale_factor = 2
        image = image.resize((image.width * scale_factor, image.height * scale_factor), Image.Resampling.LANCZOS)
    
    # 4. Sharpening to make characters stand out
...
def ocr_process():
    try:
        ...
        # Custom configuration for better Azerbaijani support
        ...
        if is_pdf:
            # Increase DPI to 300 for high-quality extraction from PDF
        ...
        return jsonify({'error': 'The server encountered an issue processing your file. Ensure it is a valid image or PDF.'}), 500
